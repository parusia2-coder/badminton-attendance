...
function showFeeSettingModal() {
  ...
  if (!modalContainer) {
    alert('modalContainer를 찾을 수 없습니다');
    return;
  }
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <div class="bg-gray-50 px-6 py-4 rounded-b-lg flex justify-end gap-3">
          ...
          <button onclick="saveFeeSetting()" class="bg-purple-600 text-white px-6 py-2 rounded-lg hover:bg-purple-700">저장</button>
        </div>
      </div>
    </div>
  `;
  ...
  modalContainer.innerHTML = `
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <div class="bg-gray-50 px-6 py-4 rounded-b-lg flex justify-end gap-3">
          <button onclick="closeModal()" class="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-100">취소</button>
          <button onclick="processFeePayment()" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">등록</button>
        </div>
      </div>
    </div>
  `;
...
async function sendUnpaidSMS() {
  ...
  alert('미납자 문자 발송 기능이 호출되었습니다');
}

console.log('✅ 모든 회비 함수 정의 완료');
