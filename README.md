...
6. **⭐ 문자발송 시스템**: NHN Cloud SMS 연동으로 회원 일괄 문자 발송
...
- **관리자(admins)**: 관리자 계정
- **문자발송이력(sms_logs)**: SMS 발송 로그 및 통계
...
### 7. 문자발송 ⭐ NEW
1. 회원관리 메뉴 클릭
2. 원하는 회원 체크박스 선택 (선택 안하면 전체 발송)
3. "문자발송" 버튼 클릭
4. 템플릿 선택 또는 직접 입력
5. 발송 전 수신자 및 예상 비용 확인
6. "발송" 버튼으로 문자 전송

**⚠️ 주의**: 실제 문자 발송을 위해서는 NHN Cloud SMS 계정 필요
- `.dev.vars` 파일에 API 키 설정
- 프로덕션: Cloudflare Pages Secrets에 등록
...
✅ 관리자 인증 (로그인/로그아웃)
✅ 모바일 반응형 디자인
✅ **⭐ 문자발송 시스템** (NHN Cloud SMS 연동)

### 문자발송 기능 상세 ⭐ NEW
- **API 연동**: NHN Cloud SMS (Toast)
- **발송 방식**: 
  - 개별 회원 선택 발송
  - 다중 회원 선택 발송
  - 전체 회원 일괄 발송
  - 필터된 회원 그룹 발송
- **템플릿**: 정기모임 안내, 회비 납부, 일정 리마인더, 공지사항
- **비용 예상**: 건당 약 9원 (NHN Cloud 기준)
- **발송 이력**: 수신자, 내용, 상태, 오류 메시지 저장
- **통계**: 일별/월별 발송 건수, 성공률
- **환경변수**: `.dev.vars` (로컬), Cloudflare Secrets (프로덕션)

### NHN Cloud SMS 설정 방법
1. NHN Cloud 가입 및 SMS 서비스 신청
2. AppKey, SecretKey, 발신번호 발급
3. `.dev.vars` 파일에 설정:
   ```
